...
if ( ! class_exists( 'Storefront_NUX_Admin' ) ) :
	...
	class Storefront_NUX_Admin {
		...
		public function enqueue_scripts() {
			...
			wp_enqueue_style( 'storefrontAS-admin-nux', get_template_directory_uri() . '/assets/css/admin/admin.css', '', $storefront_version );

			wp_enqueue_script( 'storefrontAS-admin-nux', get_template_directory_uri() . '/assets/js/admin/admin' . $suffix . '.js', array( 'jquery' ), $storefront_version, 'all' );

			$storefront_nux = array(
				'nonce' => wp_create_nonce( 'storefront_notice_dismiss' )
			);

			wp_localize_script( 'storefrontAS-admin-nux', 'storefrontNUX', $storefront_nux );
		}
		...
		public function admin_notices() {
			...
			<div class="notice notice-info sf-notice-nux is-dismissible">
				<span class="sf-icon">
					<?php echo '<img src="' . esc_url( get_template_directory_uri() ) . '/assets/images/admin/storefrontAS-icon.svg" alt="Storefront" width="250" />'; ?>
				</span>
		...
		public function redirect_customizer() {
			...
			$args['return'] = urlencode( admin_url( 'themes.php?page=storefrontAS-welcome' ) );
