...
if ( ! function_exists( 'storefront_sorting_wrapper' ) ) {
	...
	function storefront_sorting_wrapper() {
		echo '<div class="storefrontAS-sorting">';
	}
}
	...
	function storefront_single_product_pagination() {
		...
		<nav class="storefront-product-pagination" aria-label="<?php esc_attr_e( 'More products', 'storefront' ); ?>">
			<?php if ( $previous_product && $previous_product->is_visible() ) : ?>
				<?php previous_post_link( '%link', wp_kses_post( $previous_product->get_image() ) . '<span class="storefrontAS-product-pagination__title">%title</span>', $in_same_term, $excluded_terms, $taxonomy ); ?>
			<?php endif; ?>

			<?php if ( $next_product && $next_product->is_visible() ) : ?>
				<?php next_post_link( '%link', wp_kses_post( $next_product->get_image() ) . '<span class="storefrontAS-product-pagination__title">%title</span>', $in_same_term, $excluded_terms, $taxonomy ); ?>
			<?php endif; ?>
		</nav><!-- .storefrontAS-product-pagination -->
		<?php
	}
}
...
if ( ! function_exists( 'storefront_sticky_single_add_to_cart' ) ) {
	...
	function storefront_sticky_single_add_to_cart() {
		...
		wp_localize_script( 'storefrontAS-sticky-add-to-cart', 'storefront_sticky_add_to_cart_params', $params );

		wp_enqueue_script( 'storefrontAS-sticky-add-to-cart' );
		?>
			...
			</section><!-- .storefrontAS-sticky-add-to-cart -->
		<?php
	}
}
...
if ( ! function_exists( 'storefront_woocommerce_brands_homepage_section' ) ) {
	...
	function storefront_woocommerce_brands_homepage_section() {
		...
		echo '<section class="storefrontAS-product-section storefrontAS-woocommerce-brands" aria-label="' . esc_attr__( 'Product Brands', 'storefront' ) . '">';
