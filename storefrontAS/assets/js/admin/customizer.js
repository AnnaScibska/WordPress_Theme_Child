...
( function( wp, $ ) {
	...
	api.SFGuidedTour = {
		...
		_hideTour: function( remove ) {
			...
			$( 'body' ).addClass( 'sf-exiting' ).on( 'animationend.storefrontAS webkitAnimationEnd.storefrontAS', function() {
				$( this ).removeClass( 'sf-exiting' ).off( 'animationend.storefrontAS webkitAnimationEnd.storefrontAS' ).addClass( 'sf-hidden' );
				self.$container.hide();
		...
		_revealTour: function() {
			...
			$( 'body' ).addClass( 'sf-entering' ).on( 'animationend.storefrontAS webkitAnimationEnd.storefrontAS', function() {
				$( this ).removeClass( 'sf-entering' ).off( 'animationend.storefrontAS webkitAnimationEnd.storefrontAS' );
		...
		_moveContainer: function( $selector ) {
			...
			this.$container.addClass( 'sf-moving' ).css({ 'transform': 'translateY(' + parseInt( position, 10 ) + 'px)' }).on( 'transitionend.storefrontAS', function() {
				self.$container.removeClass( 'sf-moving' );
				self.$container.off( 'transitionend.storefrontAS' );
			} );
		},
